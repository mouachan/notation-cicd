...
node('maven') {
  ...
  stage 'deployInDev'
    ...
    sh "oc process -f ./template/dmn-svc.yml -n notation-svc-cicd --param APP_NAME=${artifact} --param APP_VERSION=${version} --param IMAGE_VERSION=${image_version} | oc apply -n ${namespace_dev} -f -"
  ...
  stage 'deployInAcp'
    ...
    sh "oc process -f ./template/dmn-svc.yml -n notation-svc-cicd --param APP_NAME=${artifact} --param APP_VERSION=${version} --param IMAGE_VERSION=${image_version} | oc apply -n ${namespace_acp} -f -"
}
